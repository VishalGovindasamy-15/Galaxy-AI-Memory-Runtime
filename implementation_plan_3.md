...
### H1 — Latency Hiding with Safety Margin
> *"For some block sizes achievable on our hardware, GPU compute time exceeds total transfer time by a margin greater than the measured transfer time variance, such that temporary bandwidth fluctuations do not cause GPU stalls."*

Formally:
```
M  =  T_compute − T_transfer
H1 is confirmed  ↔  M > σ_transfer

where σ_transfer is the measured standard deviation of transfer time
from exp01 + exp02 combined.
```

Why this matters: if `M = 0.2 ms` but `σ = 0.5 ms`, the GPU stalls whenever
the SSD has a bad read. A margin must exceed the noise floor to be useful.

**If TRUE** (M > σ): HAMR's pipelined streaming is robust. Proceed to Phase 2.
**If M > 0 but M < σ**: Hypothesis is marginal — pipelining helps on average
but not reliably. Document this honestly.
**If M ≤ 0**: Transfer always dominates. Document the gap, identify
which block sizes come closest, adjust block size strategy accordingly.
...
### Experiment 4 — `exp04_overlap_test.py`

**Addresses**: H1 (does theoretical overlap match real overlap?)

```
Objective:       Measure actual I/O + compute overlap with simultaneous workloads
Independent var: Block size (MB), pipeline depth
Dependent var:   Actual GPU idle %, measured vs theoretical stall
Output:          JSON + timeline plot
```
...
**Goal**: Derive closed-form equations for runtime behavior BEFORE building the simulator. The simulator implements these equations, not intuitions.
...
Experiments       ░░░░░░░░░░░░░░░░░░░░    0%  Ready to start
Runtime           ░░░░░░░░░░░░░░░░░░░░    0%  Blocked on experiments
Paper             ░░░░░░░░░░░░░░░░░░░░    0%  Blocked on runtime
```

**Next action**: Rewrite `exp01_ssd_bandwidth.py` with 7 required fixes → run → get real numbers.
